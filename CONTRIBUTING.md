# Contributing to GPU Boost Toggle
...
3. **Test it.** There is no automated test suite (this is QML + shell talking to `nvidia-smi`). At minimum: run `bash -n` on any shell script you touched, and load the plasmoid on a real Plasma 6 + NVIDIA machine to confirm the panel button, config page, and toggle still work. See the README for install steps.
